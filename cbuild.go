...
//
//
func get_replaced_variable(info BuildInfo, name string) (string, error) {
    ...
    if si != -1 {
        var e error
        str, e = replace_variable(info, str, si, true, 0)
        return str, e
    }
    return str, nil
}
...
//
func string_to_replaced_list(info BuildInfo, str string) ([]string, error) {
    sl := strings.Split(str, " ")
    for i, s := range sl {
        si := strings.Index(s, "${")
        if si != -1 {
            var e error
            s, e = replace_variable(info, s, si, true, 0)
            if e != nil {
                return []string{}, e
            }
            sl[i] = s
...
func create_archive(info BuildInfo, create_list []string, target_name string) (string, error) {
    ...
    archiver, e := get_replaced_variable(info, "archiver")
    if e != nil {
        return "", e
    }
...
// link objects
//
func create_link(info BuildInfo, create_list []string, target_name string, packager Packager) error {
    ...
    linker, e := get_replaced_variable(info, "linker")
    if e != nil {
        return e
    }
    ...
    if packager.Target != "" {
        ...
        pkgr, e := get_replaced_variable(info, "packager")
        if e != nil {
            return e
        }
        pargs, e := string_to_replaced_list(info, packager.Option)
        if e != nil {
            return e
        }
        pkg := BuildCommand{
...
func compile_files(info BuildInfo, objdir string, loaddir string, files []string) (create_list []string, e error) {
    ...
    compiler, e := get_replaced_variable(info, "compiler")
    if e != nil {
        return []string{}, e
    }
...
func build(info BuildInfo, pathname string) (result BuildResult, err error) {
    ...
    if NowTarget.Type == "library" {
        ...
        if len(create_list) > 0 || len(subdir_create_list) > 0 {
            e := create_link(info, append(create_list, subdir_create_list...), NowTarget.Name, NowTarget.Packager)
            if e != nil {
                result.success = false
                return result, e
            }
